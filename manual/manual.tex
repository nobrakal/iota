...
This program has one rule meaning that two proteins $x$ and $y$ can be linked through their $s$'s and $t$'s sites if $x$ is an Enzyme. Moreover, there is an invariant saying that if $x$ and $y$ are linked though the mentioned sites, and $x$ is an Enzyme, then $y$ must be an Enzyme.
...
(* Link two "mol" through s and g if at least one is active *)
