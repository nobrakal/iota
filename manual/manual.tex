...
\begin{quote}
  ...
  Y341 [...]. Tyrosine phosphorylation and activation of Raf-1 have been shown to be co-
  incident. However, others have been unable to detect phosphotyrosine in active Raf-1.\cite{masson}
\end{quote}
...
\paragraph{Toward a fisrot-order logic}
...
Husson worked in his thesis\cite{husson} to provide a logical framework that address these
issues by being expressive enough to represent molecular interactions while being simple enough to be ``compiled'' to a set of graph rewriting rules. Husson identified a ``safe'' subset of its logic for which we know an algorithm able to synthesize equivalent \Kappa{} rules.\\

Unfortunately, it is tedious to write directly formulas in this logic, and criterions of correction need to be carefully hand-checked. Moreover, since the previously mentioned algorithm to synthesize \Kappa{} rules is doubly exponential, hand-written formulas can lead to an enormous number of rules, frequently equivalent to a much smaller set. Hence, there was a need for an intermediate language that is still made for experts but allowing the programmer to focus on describing its system rather than dealing with the formalism of the logic.
...
Any formula can be put in a \emph{normal form}, where negation is only present on leaves. First of all, we put each formula into normal form, to simplify the guard inference.
...
The resulting structure must satisfy some properties that are not syntaxic. We must ensure that every guard is indeed a well-formed guard, and that the two parts ``ensure'' and ``maintain'' are well-formed with respect to Husson's criterion. This pass only checks if these properties are respected.
