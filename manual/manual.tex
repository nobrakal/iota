...
This lists are composed of formulas are universally quantified and must respect Husson's criterion (see Section \ref{subsec:verification}).
...
The resulting structure must satisfies some properties. We must ensure that every guard is indeed a well formed guard, and that the two parts ``ensure'' and ``maintain'' are well formed. This pass only checks if these properties are respected.
