...
Unusually, the typechecking actually transform the input program. Indeed, free variables of definitions are handled specifically since we want to try to quantify them existentially. Sadly, the language is still very rich and doing the guard inference at this stage of the compilation will be difficult. Thus, we introduce a new quantifier which mark the presence of an unbounded variable for later inference.
...
Any formula can be put in a \emph{normal form}, where negation is only present on leaves. First, we put each formula into normal form, to fall back into Husson's syntax and simplify the guard inference.
